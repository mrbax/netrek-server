...
#endif /*PUCK_FIRST */
{
	...
	if (isPressoringMe(enemy_buf)) {
		...
		(me->p_dir <= vector_direction(me->p_x, me->p_y, G_LFT, ORI_G)) &&
		(me->p_dir >= vector_direction(me->p_x, me->p_y, G_RGT, ORI_G)))
	      shot_ongoal = 1;
		...
		to_left  = vector_direction(me->p_x, me->p_y, G_LFT, KLI_G);
		to_right = vector_direction(me->p_x, me->p_y, G_RGT, KLI_G);
